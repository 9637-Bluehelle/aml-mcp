...
import { explainError } from '../lib/erroriLeggibili';
...
export function PianoApprovazione({
  ...
  async function decidi(approva: boolean) {
    ...
    try {
      ...
      if (error) { toast.error(`Operazione fallita: ${explainError(error.message)}`); return; }
      ...
      if (approva) {
        ...
        try {
          ...
          toast.error(`Esecuzione fallita: ${explainError(e)}. Il piano resta approvato: riprova.`);
        }
      } else {
