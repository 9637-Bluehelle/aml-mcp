...
// L'incarico non espone "Codice Fiscale" e "Nome cliente" come modalità a sé: per un
// dato cliente produrrebbero sempre lo stesso codice, e un cliente può avere più
// incarichi. Restano disponibili come componenti del codice (le checkbox sotto),
// dove il progressivo finale garantisce l'unicità.
const FORMATO_INCARICO_OPTIONS: FormatoOption[] = [
  { value: 'manuale', label: 'Manuale', desc: 'L\'utente inserisce il codice liberamente' },
  { value: 'sequenziale', label: 'Sequenziale', desc: 'Numerazione progressiva (001, 002, ...)' },
];
  ...
  return (
    <div className="space-y-6 max-w-3xl mx-auto">
      ...
      <SezioneCodice
        ...
        descrizione="Configura come generare automaticamente il codice incarico. Il codice può essere composto da più parti, ma termina sempre con un progressivo: è ciò che distingue più incarichi dello stesso cliente."
        opzioni={FORMATO_INCARICO_OPTIONS}
        formato={impostazioni.formato_codice_incarico === 'sequenziale_cliente' ? 'sequenziale' : impostazioni.formato_codice_incarico}
        onFormatoChange={v => {
          // FORMATO_INCARICO_OPTIONS espone solo 'manuale' e 'sequenziale'; il
          // "conta per cliente" è la checkbox dedicata, da preservare nel passaggio.
          if (v === 'sequenziale') {
            update({ formato_codice_incarico: impostazioni.formato_codice_incarico === 'sequenziale_cliente' ? 'sequenziale_cliente' : 'sequenziale' });
          } else {
            update({ formato_codice_incarico: 'manuale' });
          }
        }}
  ...
  return (
    <div className="bg-white border rounded-lg p-6 space-y-5">
      ...
      <div>
        ...
        <div className={`grid gap-2 ${opzioni.length > 2 ? 'grid-cols-2 sm:grid-cols-4' : 'grid-cols-1 sm:grid-cols-2'}`}>
          {opzioni.map(opt => (
            <button
              key={opt.value}
