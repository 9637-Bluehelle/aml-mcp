...
// ATTENZIONE prima di toccare la dipendenza `xlsx` in package.json.
//
// Punta di proposito a un tarball di cdn.sheetjs.com, NON a una versione del
// registry npm. Non è una svista: SheetJS ha smesso di pubblicare su npm dopo
// la 0.18.5, che resta ferma lì con due vulnerabilità note —
// prototype pollution (CVE-2023-30533, corretta in 0.19.3) e ReDoS (corretta
// in 0.20.2). `npm audit` la segnalava come "high" senza fix disponibile,
// perché su npm un fix davvero non c'è.
//
// Qui conta più che altrove: XLSX.read() gira sui file che l'utente carica,
// quindi su input di provenienza esterna. Il limite di 15 MB e l'allowlist di
// estensioni in ImportClientiModal fermano le zip bomb, non il prototype
// pollution, che non dipende dalla dimensione del file.
//
// Riportarla a "^0.18.5" o "latest" riaprirebbe entrambe in silenzio: il codice
// continuerebbe a compilare e i test a passare, perché il pacchetto si chiama
// `xlsx` in entrambi i casi. Per aggiornare, prendere la versione nuova dallo
// stesso CDN: https://cdn.sheetjs.com/xlsx-<versione>/xlsx-<versione>.tgz
...
/** Numero di clienti ATTIVI dello studio corrente: sono quelli che consumano gli
 *  slot del piano. Stessa definizione usata dal trigger `enforce_cliente_limit`
 *  (i clienti nel cestino non contano) e stesso scoping per studio della
 *  deduplica, così il conteggio mostrato in anteprima combacia con quello che il
 *  DB applicherà davvero. */
export async function countClientiAttivi(): Promise<number> {
  let q = supabase.from('clienti').select('id', { count: 'exact', head: true }).is('deleted_at', null);
  const studioId = getActiveStudioIdHolder();
  if (studioId) q = q.eq('studio_id', studioId);
...
export async function importClienti(
  ...
  const PLAN_LIMIT_MSG = 'Non importato: limite clienti del piano raggiunto. '
    + 'Passa a un piano superiore per importarne altri.';

  let done = 0;
  let planLimitHit = false; // una volta raggiunto il limite, le righe successive non si tentano
  for (const row of importable) {
    if (signal?.aborted) break; // annullato dall'utente: interrompi senza toccare il già fatto

    // Limite piano già raggiunto: marca come non importato senza colpire il DB.
    if (planLimitHit) {
      const outcome: ImportOutcome = {
        index: row.index,
        ragione_sociale: row.normalized.ragione_sociale,
        ok: false,
        error: PLAN_LIMIT_MSG,
      };
    ...
    try {
      ...
      // Il trigger DB enforce_cliente_limit rifiuta i clienti oltre il limite del
      // piano: da qui in poi ogni riga fallirebbe → interrompi i tentativi.
      if (emsg.includes('PLAN_LIMIT_CLIENTI')) {
        planLimitHit = true;
        outcome = {
          index: row.index,
          ragione_sociale: row.normalized.ragione_sociale,
        ...
        outcome = {
          ...
          ragione_sociale: row.normalized.ragione_sociale,
          ok: false,
          error: emsg || 'Errore sconosciuto durante l\'inserimento',
        };
      }
    }
    outcomes.push(outcome);
