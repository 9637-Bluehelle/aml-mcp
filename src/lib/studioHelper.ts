...
// Holder a modulo per lo studio dell'utente. Sincronizzato da StudioProvider,
// e serve ai moduli plain (es. personeHelper) che non hanno accesso al React
// context.
//
// Coincide sempre con lo studio del proprio profilo. Non è sempre stato così:
// il superadmin poteva commutarlo su un altro studio, ed è per quello che il
// valore vive qui invece di essere letto una volta. Chi legge continua a
// trovarlo comodo — è sincrono, dove `getMyStudioId()` è una query — ma non
// deve più aspettarsi che cambi durante la sessione.
...
export async function getMyStudio(): Promise<{
  ...
  const { data } = await supabase
    .from('studi')
    .select('id, nome, created_by, comune_sede, provincia_sede, via_piazza_sede, numero_civico_sede, cap_sede, nome_proprietario, cognome_proprietario, albo_sede, albo_numero, albo_sezione, delega_admin_av5, piano, piano_stato')
    .eq('id', studioId)
    .single();
