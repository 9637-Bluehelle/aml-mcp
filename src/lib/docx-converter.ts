...
interface Cliente {
  ...
  /** Documento di identità del cliente persona fisica (colonna JSONB di `clienti`) */
  documento_identita?: {
    tipo?: string | null;
    numero?: string | null;
  ...
  pep_verificato?: boolean | null;
  pep_data_verifica?: string | null;
  sanzioni_verificato?: boolean | null;
  sanzioni_data_verifica?: string | null;
}

interface TitolareEffettivo {
  nome_cognome: string;
  codice_fiscale: string | null;
  /** Professione anagrafica della persona (vive su anagrafica_soggetti). */
  professione: string | null;
  /**
   * Ruolo nel rapporto con QUESTO cliente (vive su titolari_effettivi): è il
   * dato che i documenti devono riportare sotto "Ruolo". Facoltativo perché i
   * titolari salvati prima della separazione dei due campi hanno il ruolo
   * ancora dentro `professione` — da cui il fallback nei punti di stampa.
   */
  ruolo?: string | null;
...
interface Incarico {
  ...
  /** Data di cessazione della prestazione — riga "cessazione" dell'AV.2 completo */
...
export interface DocumentoAllegato {
  ...
  nome_file?: string;
  label?: string;
  /** Data di assunzione al fascicolo — stampata nelle annotazioni dell'AV.2 */
...
// Utility: Tipo di adeguata verifica per classe di rischio
// Rif.: D.Lgs. 231/2007, artt. 23 (semplificata), 18 (ordinaria), 24-25 (rafforzata)
function getTipoAdeguataVerifica(classe: number): string {
  if (classe >= 4) return 'Rafforzata';
  if (classe === 3) return 'Ordinaria';
  return 'Semplificata';
}
          ...
          ? titolari_effettivi.slice(0, 9).flatMap((titolare, index) => {
              ...
              new Paragraph({
                children: [
                  ...
                  new TextRun({ text: titolare.ruolo || titolare.professione || 'N/D' })
                ],
                spacing: { after: 80 }
              }),
              ...(titolare.residenza ? [
          ...
          ? titolari_effettivi.flatMap((titolare, index) => {
              ...
              new Paragraph({
                children: titolare.tipo_rapporto === 'societa_ente'
                  ...
                  : [new TextRun({ text: tIsAzienda ? 'Attività: ' : 'Ruolo: ', bold: true }), new TextRun({ text: titolare.ruolo || titolare.professione || 'N/D' })],
                spacing: { after: 80 },
                indent: { left: 250, hanging: 0 }
              }),
...
// ============================================================
// AV.2 — Documento riepilogativo dati, informazioni e documenti acquisiti
// ============================================================

/**
 * Larghezze colonne (dxa) prese pari pari dal `w:tblGrid` del modello CNDCEC.
 * Totale 9628 = larghezza utile della pagina del modello (11906 - 1134*2 margini).
 */
const AV2_COLS = [400, 3205, 3332, 2691] as const;
const AV2_TABLE_W = AV2_COLS.reduce((a, b) => a + b, 0);

/** TextRun con il corpo tabella del modello AV.2 (8pt) */
function av2Text(text: string, opts?: { bold?: boolean; italic?: boolean; size?: number; color?: string }): TextRun {
  return new TextRun({
    text,
    bold: opts?.bold,
    italics: opts?.italic,
...
/** Paragrafo giustificato, come tutte le celle del modello AV.2 */
function av2Para(
  runs: TextRun[],
...
function av2Cell(
  ...
  return new TableCell({
    ...
    children: children.length > 0 ? children : [av2Para([])],
  });
}

/** Celle "DOCUMENTAZIONE"/"OSSERVAZIONI": testo fisso del modello, una riga per paragrafo */
function av2FixedCell(paragraphs: string[], width: number): TableCell {
  return av2Cell(paragraphs.map(p => av2Para([av2Text(p)])), width);
}

/** Cella "(X)": casella spuntata/vuota, centrata */
function av2CheckCell(checked: boolean): TableCell {
  return new TableCell({
    width: { size: AV2_COLS[0], type: WidthType.DXA },
    verticalAlign: VerticalAlign.CENTER,
    margins: { left: 40, right: 40, top: 40, bottom: 40 },
    children: [
      av2Para([av2Text(getCheckbox(checked), { size: 20, bold: checked })], { align: AlignmentType.CENTER }),
    ],
  });
}

/** Cella "ANNOTAZIONI PROFESSIONISTA": righe precompilate, oppure vuota se non c'è nulla da dire */
function av2NotesCell(lines: string[]): TableCell {
  return av2Cell(
    lines.map(l => av2Para([av2Text(l)])),
    AV2_COLS[3]
  );
}

/** Opzioni di precompilazione dell'AV.2, derivate dalla check-list digitale del fascicolo. */
export interface AV2Options {
  /**
   * Stato della check-list AV.2 digitale: `{ ...clientChecks, ...incaricoChecks }`.
   * Le chiavi sono gli id degli item definiti in `CHECKLIST_AV2` (FascicoloCliente).
   */
  checks: Record<string, boolean>;
  /** Classe di rischio effettivo dell'incarico (null se nessuna valutazione) */
  classeRischio?: number | null;
  /** Data del prossimo controllo costante programmato (ISO) */
  prossimoControllo?: string | null;
  /** Titolari effettivi con documento di identità acquisito, su totale */
  titolariDocumenti?: { done: number; total: number };
}

/** Tipologie già rendicontate nelle righe 1-6 del modello: non vanno ripetute in "Altra documentazione" */
const AV2_TIPOLOGIE_RIGHE_FISSE = new Set([
  'documento_identita',
...
/**
 * Genera documento DOCX AV.2 - Documento riepilogativo dati, informazioni e documenti acquisiti
 *
 * Struttura fedele al modello in documentazione/CNDCEC_modulistica-AML-1 (1).docx:
 * tabella a 4 colonne — (X) | DOCUMENTAZIONE | OSSERVAZIONI | ANNOTAZIONI PROFESSIONISTA —
 * con le 6 righe di documentazione tipica più la riga libera "Altra documentazione".
 * Testi delle colonne DOCUMENTAZIONE/OSSERVAZIONI riportati alla lettera dal modello;
 * la colonna (X) e la colonna ANNOTAZIONI sono precompilate con lo stato della
 * check-list AV.2 digitale del fascicolo.
 */
export async function generateBlobDOCX_AV2(
  data: AMLData,
  options: AV2Options
): Promise<{ blob: Blob; filename: string }> {
  const { cliente, incarico, valutazione } = data;
  const { checks, classeRischio = null, prossimoControllo = null, titolariDocumenti } = options;
  const today = formatDate(new Date().toISOString());
  const documenti = data.documenti || [];

  const isPF = cliente.tipo_cliente === 'persona_fisica' || cliente.tipo_cliente === 'professionista';

  /** "nome file — acquisito il gg/mm/aaaa", come da nota 1 del modello */
  const docLine = (d: DocumentoAllegato): string => {
    const base = d.nome_file?.trim() || d.label || TIPOLOGIA_LABELS[d.tipologia] || d.tipologia;
    const dt = d.data_acquisizione ? formatDate(d.data_acquisizione) : 'N/D';
    return dt !== 'N/D' ? `${base} — acquisito il ${dt}` : base;
  };
  const docsOf = (...tipi: string[]): DocumentoAllegato[] => documenti.filter(d => tipi.includes(d.tipologia));

  // =================== RIGA 1 — Documento di identità ===================
  const noteIdentita: string[] = [];
  const docIdent = isPF ? cliente.documento_identita : null;
  const rappDoc = cliente.rappresentante_legale_documento;
  const fmtDocumento = (
    doc: { tipo?: string | null; numero?: string | null; data_scadenza?: string | null; ente_rilascio?: string | null },
    intestatario: string
  ): string => {
    const tipo = doc.tipo === 'carta-identita' ? 'Carta d\'Identità' : (doc.tipo || 'Documento');
    const parti = [`${tipo} n. ${doc.numero || 'N/D'}`];
    if (doc.ente_rilascio) parti.push(`rilasciato da ${doc.ente_rilascio}`);
    if (doc.data_scadenza) parti.push(`scadenza ${formatDate(doc.data_scadenza)}`);
    return `${intestatario}: ${parti.join(', ')}`;
  };
  if (docIdent?.tipo || docIdent?.numero) {
    noteIdentita.push(fmtDocumento(docIdent, cliente.ragione_sociale || 'Cliente'));
  }
  if (!isPF && (rappDoc?.tipo || rappDoc?.numero)) {
    noteIdentita.push(fmtDocumento(rappDoc, cliente.rappresentante_legale || 'Legale rappresentante/esecutore'));
  }
  docsOf('documento_identita', 'documento_identita_esecutore').forEach(d => noteIdentita.push(docLine(d)));

  // =================== RIGA 2 — Visura / atti costitutivi ===================
  // La riga copre visura e atti costitutivi come alternative ("oppure" nel modello):
  // si spunta sul documento effettivamente acquisito. Per le persone fisiche non si
  // applica, quindi casella vuota e annotazione esplicita anziché spunta.
  const noteVisura: string[] = [];
  const haVisuraOAtti = docsOf('visura', 'atti_costitutivi').length > 0;
  if (isPF) {
    noteVisura.push(
      cliente.tipo_cliente === 'professionista'
        ? 'Non applicabile — Cliente professionista non iscritto al Registro delle Imprese.'
        : 'Non applicabile — Cliente persona fisica.'
    );
  } else {
    docsOf('visura', 'atti_costitutivi').forEach(d => noteVisura.push(docLine(d)));
  }

  // =================== RIGA 3 — Codice fiscale / partita IVA ===================
  const noteCF: string[] = [];
  if (cliente.codice_fiscale) noteCF.push(`Codice fiscale: ${cliente.codice_fiscale}`);
  if (cliente.partita_iva) noteCF.push(`Partita IVA: ${cliente.partita_iva}`);
  docsOf('codice_fiscale').forEach(d => noteCF.push(docLine(d)));

  // =================== RIGA 4 — Dichiarazione ex art. 22 (AV.4) ===================
  const noteAv4: string[] = [];
  const docsAv4 = docsOf('dichiarazione_av4');
  if (docsAv4.length > 0) {
    docsAv4.forEach(d => noteAv4.push(docLine(d)));
  } else if (checks['cl_dichiarazione_av4']) {
    noteAv4.push('Dichiarazione ex art. 22 acquisita agli atti (format AV.4).');
  }

  // =================== RIGA 5 — Scheda rischio effettivo (AV.1) ===================
  const noteAv1: string[] = [];
  if (valutazione) {
    noteAv1.push(`Scheda AV.1 del ${formatDate(valutazione.created_at)}.`);
    noteAv1.push(
      `Rischio effettivo: ${Number(valutazione.rischio_effettivo).toFixed(2)} — ${getClassificationText(Number(valutazione.rischio_effettivo))}.`
    );
    if (valutazione.classe_rischio) {
      noteAv1.push(`Classe ${valutazione.classe_rischio} — adeguata verifica ${getTipoAdeguataVerifica(valutazione.classe_rischio)}.`);
    }
  }
  const scadenzaControllo = prossimoControllo || valutazione?.prossimo_controllo || null;
  if (scadenzaControllo) noteAv1.push(`Prossimo controllo costante: ${formatDate(scadenzaControllo)}.`);

  // =================== RIGA 6 — Scheda di adeguata verifica (AV.3) ===================
  const noteAv3: string[] = [];
  if (checks['cl_scheda_av3']) {
    noteAv3.push('Scheda AV.3 generata dalla piattaforma sui dati del fascicolo.');
    if (incarico.codice_incarico) noteAv3.push(`Incarico ${incarico.codice_incarico}.`);
  } else {
    noteAv3.push('Dati del fascicolo incompleti: scheda AV.3 da completare.');
  }

  // =================== RIGA 7 — Altra documentazione ===================
  // Documenti del fascicolo non già rendicontati nelle righe 1-6...
  const altriDocumenti = documenti
    .filter(d => !AV2_TIPOLOGIE_RIGHE_FISSE.has(d.tipologia))
    .map(d => {
      const tipo = d.label || TIPOLOGIA_LABELS[d.tipologia] || d.tipologia;
      const nome = d.nome_file?.trim();
      const dt = d.data_acquisizione ? formatDate(d.data_acquisizione) : 'N/D';
      const suffix = dt !== 'N/D' ? ` — acquisito il ${dt}` : '';
      return nome ? `${tipo}: ${nome}${suffix}` : `${tipo}${suffix}`;
    });

  // ...più gli adempimenti della check-list che non sono file allegati.
  const altriAdempimenti: string[] = [];
  if (checks['cl_titolare_effettivo'] && !isPF) {
    const conDoc = titolariDocumenti;
    altriAdempimenti.push(
      conDoc && conDoc.total > 0
        ? `Titolari effettivi individuati e verificati: ${conDoc.total} — documenti di identità acquisiti per ${conDoc.done} su ${conDoc.total}.`
        : 'Titolari effettivi individuati e verificati.'
    );
  }
  if (checks['cl_pep_check']) {
    const dt = cliente.pep_data_verifica ? formatDate(cliente.pep_data_verifica) : 'N/D';
    altriAdempimenti.push(
      `Verifica persona politicamente esposta (PPE) effettuata${dt !== 'N/D' ? ` il ${dt}` : ''} — esito: ${cliente.pep ? 'PPE' : 'non PPE'}.`
    );
  }
  if (checks['cl_sanzioni_check']) {
    const dt = cliente.sanzioni_data_verifica ? formatDate(cliente.sanzioni_data_verifica) : 'N/D';
    altriAdempimenti.push(`Verifica liste sanzioni/embargo effettuata${dt !== 'N/D' ? ` il ${dt}` : ''}.`);
  }
  if (checks['cl_scopo_natura'] && incarico.scopo_natura) {
    altriAdempimenti.push(`Scopo e natura della prestazione: ${incarico.scopo_natura}`);
  }
  if (checks['cl_mezzi_pagamento'] && incarico.mezzi_pagamento) {
    altriAdempimenti.push(`Mezzi di pagamento: ${incarico.mezzi_pagamento}`);
  }
  if (checks['cl_provenienza_fondi'] && incarico.provenienza_fondi) {
    altriAdempimenti.push(`Provenienza dei fondi: ${incarico.provenienza_fondi}`);
  }
  if (checks['cl_controllo_costante'] && scadenzaControllo) {
    altriAdempimenti.push(`Controllo costante programmato per il ${formatDate(scadenzaControllo)}.`);
  }

  const vociAltro = [...altriDocumenti, ...altriAdempimenti];
  const hasAltro = vociAltro.length > 0;

  // Il modello lascia 10 righe bianche sotto "(specificare):": le riempiamo con le voci
  // disponibili e manteniamo le restanti vuote per la compilazione manuale.
  const RIGHE_LIBERE_ALTRO = 10;
  const paragrafiAltro: Paragraph[] = [
    av2Para([av2Text('Altra documentazione ritenuta opportuna a seguito di valutazioni/considerazioni del Professionista (specificare):')]),
    ...vociAltro.map(v => av2Para([av2Text(`- ${v}`)])),
    ...Array.from({ length: Math.max(0, RIGHE_LIBERE_ALTRO - vociAltro.length) }, () => av2Para([])),
  ];
  ...
  const headerRow = new TableRow({
    tableHeader: true,
    children: [
      av2Cell([av2Para([av2Text('(X)')], { align: AlignmentType.CENTER })], AV2_COLS[0], { vAlign: VerticalAlign.CENTER }),
      av2Cell([av2Para([av2Text('DOCUMENTAZIONE', { bold: true })], { align: AlignmentType.CENTER })], AV2_COLS[1], { vAlign: VerticalAlign.CENTER }),
      av2Cell([av2Para([av2Text('OSSERVAZIONI', { bold: true })], { align: AlignmentType.CENTER })], AV2_COLS[2], { vAlign: VerticalAlign.CENTER }),
      av2Cell([av2Para([av2Text('ANNOTAZIONI PROFESSIONISTA', { bold: true })], { align: AlignmentType.CENTER })], AV2_COLS[3], { vAlign: VerticalAlign.CENTER }),
    ],
  });
  ...
  const rows: TableRow[] = [
    ...
    new TableRow({
      children: [
        av2CheckCell(!!checks['cl_identita']),
        av2FixedCell(
        ...
        av2FixedCell(
          ['Documento del Cliente persona fisica ovvero dell’esecutore (soggetto che agisce per conto del Cliente società/ente)'],
          AV2_COLS[2]
        ),
        av2NotesCell(noteIdentita),
      ],
    }),
    // Riga 2 — visura / atti costitutivi
    new TableRow({
      children: [
        av2CheckCell(!isPF && haVisuraOAtti),
        av2FixedCell(
          [
            'Visura del Registro Imprese (certificato equivalente per società di diritto estero) ',
            'oppure ',
            'per i soggetti/enti che non sono tenuti all’iscrizione al Registro delle Imprese, atti costitutivi, delibere o altri documenti',
          ],
          AV2_COLS[1]
        ),
        av2FixedCell(
          ['Per verificare dati e informazioni riferiti alla società/ente Cliente fra cui l’esistenza dei poteri di rappresentanza del soggetto che agisce per conto della società/ente nel conferimento dell’incarico professionale'],
          AV2_COLS[2]
        ),
        av2NotesCell(noteVisura),
      ],
    }),
    // Riga 3 — codice fiscale / partita IVA
    new TableRow({
      height: { value: 555, rule: HeightRule.ATLEAST },
      children: [
        av2CheckCell(!!checks['cl_codice_fiscale']),
        av2FixedCell(['Documentazione attestante codice fiscale e (eventuale) partita IVA'], AV2_COLS[1]),
        av2FixedCell(['Ove non presenti in altri atti acquisiti dal Professionista'], AV2_COLS[2]),
        av2NotesCell(noteCF),
      ],
    }),
    // Riga 4 — dichiarazione ex art. 22 (AV.4)
    new TableRow({
      children: [
        av2CheckCell(!!checks['cl_dichiarazione_av4']),
        av2FixedCell(['Dichiarazione antiriciclaggio resa dal Cliente ex art. 22 d.lgs. 231/2007'], AV2_COLS[1]),
        av2FixedCell(
          ['Si suggerisce l’utilizzo dell’apposito format AV.4 contenuto nella presente modulistica o altro avente contenuto analogo'],
          AV2_COLS[2]
        ),
        av2NotesCell(noteAv4),
      ],
    }),
    // Riga 5 — scheda rischio effettivo (AV.1)
    new TableRow({
      children: [
        av2CheckCell(!!checks['cl_scheda_av1']),
        av2FixedCell(['Scheda di determinazione del rischio effettivo ex art. 17 d.lgs. 231/2007'], AV2_COLS[1]),
        av2FixedCell(
          ['Si suggerisce l’utilizzo dell’apposito format AV.1 contenuto nella presente modulistica o altro avente contenuto analogo'],
          AV2_COLS[2]
        ),
        av2NotesCell(noteAv1),
      ],
    }),
    // Riga 6 — scheda di adeguata verifica (AV.3)
    new TableRow({
      children: [
        av2CheckCell(!!checks['cl_scheda_av3']),
        av2FixedCell(['Scheda di adeguata verifica'], AV2_COLS[1]),
        av2FixedCell(['È possibile utilizzare il format AV.3 o altro avente contenuto informativo analogo '], AV2_COLS[2]),
        av2NotesCell(noteAv3),
      ],
    }),
    // Riga 7 — altra documentazione
    new TableRow({
      children: [
        av2CheckCell(hasAltro),
        av2Cell(paragrafiAltro, AV2_COLS[1]),
        av2FixedCell(
          ['Ogni ulteriore documento o informazione necessaria o utile per l’adeguata verifica, ovvero per altri presidi antiriciclaggio'],
          AV2_COLS[2]
        ),
        av2NotesCell([]),
      ],
  ...
  // =================== TITOLO (con richiamo alla nota del modello) ===================
  elems.push(
    new Paragraph({
      alignment: AlignmentType.JUSTIFIED,
      spacing: { before: 200, after: 120 },
      children: [
        av2Text('AV.2 - DOCUMENTO RIEPILOGATIVO DATI, INFORMAZIONI E DOCUMENTI ACQUISITI', { bold: true, size: 26 }),
        new FootnoteReferenceRun(1),
      ],
    })
  );

  // =================== IDENTIFICAZIONE (precompilata) ===================
  // Il modello non prevede questi campi: li riportiamo in testa perché il riepilogo
  // viene prodotto per singolo incarico e deve essere riconducibile al fascicolo.
  const clienteLabel = cliente.ragione_sociale || cliente.rappresentante_legale || '';
  const idIncarico = [incarico.codice_incarico, incarico.descrizione || incarico.scopo_natura].filter(Boolean).join(' — ');
  const classeFinale = classeRischio ?? valutazione?.classe_rischio ?? null;
  if (clienteLabel) {
    elems.push(
      av2Para([av2Text('Cliente: ', { size: 18 }), av2Text(clienteLabel, { size: 18, bold: true })], { after: 40 })
    );
  }
  if (idIncarico) {
    elems.push(
      av2Para([av2Text('Incarico: ', { size: 18 }), av2Text(idIncarico, { size: 18, bold: true })], { after: 40 })
    );
  }
  if (classeFinale) {
    elems.push(
      av2Para(
        [
          av2Text('Adeguata verifica: ', { size: 18 }),
          av2Text(`${getTipoAdeguataVerifica(classeFinale)} (classe ${classeFinale})`, { size: 18, bold: true }),
        ],
        { after: 200 }
      )
    );
    ...
    elems.push(
      av2Para(
        [av2Text('Adeguata verifica: nessuna valutazione del rischio presente a sistema.', { size: 18, italic: true })],
        { after: 200 }
      )
    );
  }
  ...
  elems.push(
    new Table({
      width: { size: AV2_TABLE_W, type: WidthType.DXA },
      columnWidths: [...AV2_COLS],
  ...
  // =================== DATA E FIRMA ===================
  const nomeCognomeProf = [data.studio_nome_proprietario?.trim(), data.studio_cognome_proprietario?.trim()]
    .filter(Boolean)
    .join(' ');
  elems.push(
    av2Para([], { after: 200 }),
    av2Para([av2Text('Data ', { size: 20 }), av2Text(today, { size: 20, bold: true })], { after: 200 }),
    av2Para(
      [
        av2Text('Il Professionista ', { size: 20 }),
        nomeCognomeProf
          ? av2Text(nomeCognomeProf, { size: 20, bold: true })
          : av2Text('______________________________________', { size: 20 }),
      ],
      { after: 200 }
    )
  );
  ...
  const doc = new Document({
      ...
      1: {
        children: [
          new Paragraph({
            ...
            children: [
              av2Text(
                ' Tale modulo ha finalità meramente riepilogativa, utile a indicare la documentazione acquisita ai fini antiriciclaggio con la relativa data di assunzione al fascicolo; pertanto, la sua compilazione NON COSTITUISCE ADEMPIMENTO DI UN OBBLIGO DI LEGGE e la scelta di adottarlo è rimessa unicamente ad una valutazione in termini di opportunità operata dal Professionista.',
                { size: 16 }
              ),
            ],
          }),
    ...
    sections: [
      {
        properties: {
          // Margini del modello CNDCEC: con 1134 twip a sinistra/destra la tabella
          // da 9628 dxa occupa esattamente la larghezza utile della pagina A4.
          page: { margin: { top: 1417, right: 1134, bottom: 1134, left: 1134 } },
        },
        children: elems,
      },
    ],
  ...
  const blob = await Packer.toBlob(doc);
  const filename = `AV2_Riepilogo_${incarico.codice_incarico || today.replace(/\//g, '-')}.docx`;
  return { blob, filename };
}

export async function generateAndDownloadDOCX_AV2(data: AMLData, options: AV2Options): Promise<void> {
  const { blob, filename } = await generateBlobDOCX_AV2(data, options);
  saveAs(blob, filename);
}

// ============================================================
// AV.2 COMPLETO — Check-list ai fini della formazione del fascicolo del cliente
// (versione estesa delle Linee Guida CNDCEC, 22 voci)
// ============================================================

/** Larghezze colonne (dxa) dal `w:tblGrid` di ALLEGATI_Linee_Guida.docx — stesso totale della versione sintetica */
const AV2LG_COLS = [396, 3206, 3334, 2692] as const;
const AV2LG_TABLE_W = AV2LG_COLS.reduce((a, b) => a + b, 0);

/** Sfondi originali del modello: azzurro su DOCUMENTAZIONE, ambra su OSSERVAZIONI */
const AV2LG_SHD_DOC = 'DEEAF6';
const AV2LG_SHD_OSS = 'FFF2CC';

/** Una riga della check-list estesa. `checked: null` = riga separatore, senza casella. */
interface AV2LGRow {
  checked: boolean | null;
  /** Paragrafi della colonna DOCUMENTAZIONE (testo letterale del modello) */
  doc: string[];
  /** Primo paragrafo in grassetto (righe di intestazione logica del modello) */
  docBold?: boolean;
  /** Paragrafi della colonna OSSERVAZIONI (testo letterale del modello) */
  oss: string[];
  /** Ultimo paragrafo di `oss` in grassetto */
  ossLastBold?: boolean;
  /** Sotto-elenco puntato in coda alle OSSERVAZIONI (usato dalla riga AV.3) */
  ossBullets?: string[];
  /** Annotazioni precompilate dai dati del fascicolo */
  note: string[];
  /** Disattiva gli sfondi colorati (riga separatore "(in alternativa)") */
  plain?: boolean;
}

/**
 * Genera documento DOCX AV.2 COMPLETO - Check-list ai fini della formazione del fascicolo del cliente
 *
 * Struttura fedele al modello in documentazione/ALLEGATI_Linee_Guida.docx:
 * tabella a 4 colonne — (X) | DOCUMENTAZIONE | OSSERVAZIONI | ANNOTAZIONI PROFESSIONISTA —
 * con 24 righe (22 voci spuntabili + 2 righe separatore sulla Regola Tecnica 2.1).
 * È la versione su cui è modellata la check-list digitale del fascicolo: a differenza
 * del modulo sintetico della modulistica CNDCEC, ogni adempimento tracciato dalla
 * piattaforma ha qui una riga propria.
 */
export async function generateBlobDOCX_AV2Completo(
  data: AMLData,
  options: AV2Options
): Promise<{ blob: Blob; filename: string }> {
  const { cliente, incarico, valutazione } = data;
  ...
  const docsOf = (...tipi: string[]): DocumentoAllegato[] => documenti.filter(d => tipi.includes(d.tipologia));
  const linesOf = (...tipi: string[]): string[] => docsOf(...tipi).map(docLine);
  const nonApplicabilePF = isPF
    ? cliente.tipo_cliente === 'professionista'
      ? ['Non applicabile — Cliente professionista non iscritto al Registro delle Imprese.']
      : ['Non applicabile — Cliente persona fisica.']
    : [];

  // Le righe "Visura" e "Atti costitutivi" del modello esteso sono alternative fra loro:
  // la seconda vale espressamente per i soggetti NON tenuti all'iscrizione al Registro.
  // Vanno quindi spuntate sul documento effettivamente presente, non sui flag della
  // check-list digitale, che invece li fonde (`cl_atti_costitutivi` è vero anche con la
  // sola visura, perché lì la voce copre entrambe le ipotesi).
  const haVisura = docsOf('visura').length > 0;
  const haAttiCostitutivi = docsOf('atti_costitutivi').length > 0;

  // --- Riga 1: prestazioni a rischio inerente "non significativo" (RT 2.1, Tabella 1) ---
  const rischioInerente = valutazione ? Number(valutazione.rischio_inerente_prestazione) : null;
  const isRischioNonSignificativo = rischioInerente !== null && !isNaN(rischioInerente) && rischioInerente < 1.6;
  const noteRT2: string[] = [];
  if (rischioInerente !== null && !isNaN(rischioInerente)) {
    noteRT2.push(`Rischio inerente della prestazione: ${rischioInerente.toFixed(2)} — ${getClassificationText(rischioInerente)}.`);
    noteRT2.push(
      isRischioNonSignificativo
        ? 'Rientra nella Tabella 1 della Regola Tecnica 2.1: documentazione ulteriore non necessaria.'
        : 'Non rientra nella Tabella 1 della Regola Tecnica 2.1: si applica l\'elenco sottostante.'
    );
  }

  // --- Riga documento di identità ---
  const noteIdentita: string[] = [];
  const docIdent = isPF ? cliente.documento_identita : null;
  const rappDoc = cliente.rappresentante_legale_documento;
  const fmtDocumento = (
  ...
  if (docIdent?.tipo || docIdent?.numero) noteIdentita.push(fmtDocumento(docIdent, cliente.ragione_sociale || 'Cliente'));
  if (!isPF && (rappDoc?.tipo || rappDoc?.numero)) {
    noteIdentita.push(fmtDocumento(rappDoc, cliente.rappresentante_legale || 'Legale rappresentante/esecutore'));
  }
  noteIdentita.push(...linesOf('documento_identita', 'documento_identita_esecutore'));
  if (checks['cl_pep_check']) {
    const dt = cliente.pep_data_verifica ? formatDate(cliente.pep_data_verifica) : 'N/D';
    noteIdentita.push(`Verifica PPE${dt !== 'N/D' ? ` del ${dt}` : ''} — esito: ${cliente.pep ? 'PPE' : 'non PPE'}.`);
  }

  // --- Riga codice fiscale / partita IVA ---
  const noteCF: string[] = [];
  if (cliente.codice_fiscale) noteCF.push(`Codice fiscale: ${cliente.codice_fiscale}`);
  if (cliente.partita_iva) noteCF.push(`Partita IVA: ${cliente.partita_iva}`);
  noteCF.push(...linesOf('codice_fiscale'));

  // --- Riga mandato professionale ---
  const noteMandato: string[] = [...linesOf('mandato', 'incarico')];
  if (incarico.data_inizio) noteMandato.push(`Data inizio incarico: ${formatDate(incarico.data_inizio)}.`);

  // --- Riga scheda di adeguata verifica (AV.3) ---
  const noteAv3: string[] = [];
  if (checks['cl_scheda_av3']) {
    noteAv3.push('Scheda AV.3 generata dalla piattaforma sui dati del fascicolo.');
    if (incarico.codice_incarico) noteAv3.push(`Incarico ${incarico.codice_incarico}.`);
  ...
  if (checks['cl_titolare_effettivo']) {
    noteAv3.push(
      isPF
        ? 'Titolare effettivo coincidente con il Cliente persona fisica.'
        : titolariDocumenti && titolariDocumenti.total > 0
          ? `Titolari effettivi individuati: ${titolariDocumenti.total} — documenti acquisiti per ${titolariDocumenti.done} su ${titolariDocumenti.total}.`
          : 'Titolari effettivi individuati e verificati.'
    );
  }
  if (checks['cl_scopo_natura'] && incarico.scopo_natura) noteAv3.push(`Scopo e natura: ${incarico.scopo_natura}`);

  // --- Riga scheda rischio effettivo (AV.1) ---
  const noteAv1: string[] = [];
  if (valutazione) {
    noteAv1.push(`Scheda AV.1 del ${formatDate(valutazione.created_at)}.`);
    noteAv1.push(`Rischio effettivo: ${Number(valutazione.rischio_effettivo).toFixed(2)} — ${getClassificationText(Number(valutazione.rischio_effettivo))}.`);
  }

  // --- Riga obblighi semplificati / rafforzati ---
  const classeFinale = classeRischio ?? valutazione?.classe_rischio ?? null;
  const noteSempRaff: string[] = [];
  if (classeFinale) noteSempRaff.push(`Adeguata verifica ${getTipoAdeguataVerifica(classeFinale)} — classe di rischio ${classeFinale}.`);
  noteSempRaff.push(...linesOf('doc_semplificati_rafforzati'));

  // --- Riga antiterrorismo / liste sanzioni ---
  const noteSanzioni: string[] = [];
  if (checks['cl_sanzioni_check']) {
    const dt = cliente.sanzioni_data_verifica ? formatDate(cliente.sanzioni_data_verifica) : 'N/D';
    noteSanzioni.push(`Verifica liste sanzioni/embargo effettuata${dt !== 'N/D' ? ` il ${dt}` : ''}.`);
  }

  // --- Riga controllo costante ---
  const scadenzaControllo = prossimoControllo || valutazione?.prossimo_controllo || null;
  const noteControllo: string[] = [];
  if (scadenzaControllo) noteControllo.push(`Prossimo controllo costante programmato per il ${formatDate(scadenzaControllo)}.`);

  // --- Riga "operazioni" ex art. 1 lett. t) ---
  const noteOperazioni: string[] = [];
  if (incarico.mezzi_pagamento) noteOperazioni.push(`Mezzi di pagamento: ${incarico.mezzi_pagamento}`);
  if (incarico.provenienza_fondi) noteOperazioni.push(`Provenienza dei fondi: ${incarico.provenienza_fondi}`);
  if (incarico.importo_stimato) noteOperazioni.push(`Importo stimato: € ${formatCurrency(incarico.importo_stimato)}`);
  noteOperazioni.push(...linesOf('mezzi_pagamento', 'provenienza_fondi'));

  // --- Riga cessazione della prestazione ---
  const noteCessazione: string[] = [];
  if (incarico.data_fine) noteCessazione.push(`Prestazione cessata il ${formatDate(incarico.data_fine)}.`);

  // --- Riga documenti da pubblici registri (discrezionali) ---
  const notePubbliciRegistri = linesOf('procura', 'contratto');

  // --- Riga "Altra documentazione": tutto ciò che non ha trovato posto nelle righe precedenti ---
  const TIPOLOGIE_MAPPATE = new Set([
    'documento_identita', 'documento_identita_esecutore', 'visura', 'atti_costitutivi', 'codice_fiscale',
    'mandato', 'incarico', 'dichiarazione_av4', 'attestazione_av5', 'astensione_av6', 'doc_semplificati_rafforzati',
    'dichiarazione_penale', 'esiti_ricerche', 'consistenza_patrimoniale', 'bilancio', 'visura_nominativa',
    'posizione_giuridica', 'mezzi_pagamento', 'provenienza_fondi', 'procura', 'contratto',
  ]);
  const altriDocumenti = documenti
    .filter(d => !TIPOLOGIE_MAPPATE.has(d.tipologia))
    .map(d => {
      const tipo = d.label || TIPOLOGIA_LABELS[d.tipologia] || d.tipologia;
      const nome = d.nome_file?.trim();
      const dt = d.data_acquisizione ? formatDate(d.data_acquisizione) : 'N/D';
  ...
  // ================== DEFINIZIONE DELLE 24 RIGHE ==================
  const righe: AV2LGRow[] = [
    {
      checked: isRischioNonSignificativo,
      doc: ['Documento previsto dalle regole di condotta di cui alla Regola Tecnica n. 2'],
      docBold: true,
      oss: [
        'Per le prestazioni professionali a rischio inerente “non significativo” vedi specifiche previsioni contenute nella Tabella n. 1 della Regola Tecnica 2.1.',
        'In questi casi, e nei limiti previsti dalla suddetta Regola Tecnica, non è necessaria l’ulteriore documentazione di cui alla lista sottostante',
      ],
      ossLastBold: true,
      note: noteRT2,
    },
    {
      checked: null,
      doc: ['(in alternativa) ', 'PER LE PRESTAZIONI DIVERSE DA QUELLE DI CUI ALLA REGOLA TECNICA 2.1 TABELLA 1: '],
      oss: [],
      note: [],
      plain: true,
    },
    {
      checked: !!checks['cl_identita'],
      doc: ['Fotocopia documento di identità o di altro documento di riconoscimento equipollente, in corso di validità, del Cliente ovvero dell’esecutore in caso di società/enti'],
      oss: [
        'Documento del Cliente persona fisica ovvero dell’esecutore (soggetto che agisce per conto del Cliente società/ente).',
        'Da integrare eventualmente con documentazione da fonti affidabili e indipendenti per verifica dati identificativi.',
        'Annotazioni/Informazioni aggiuntive su PPE.',
        'Consultazione del seguente sito nel caso sussistano dubbi sulla veridicità del documento esibito: https://www.crimnet.dcpc.interno.gov.it/crimnet/',
      ],
      note: noteIdentita,
    },
    {
      checked: !isPF && haVisura,
      doc: ['Visura del Registro Imprese (certificato equivalente per società di diritto estero)'],
      oss: ['Per verificare denominazione/ragione sociale e sede della società/ente Cliente nonché per verificare esistenza e ampiezza dei poteri di rappresentanza del soggetto che agisce per conto della società/ente nel conferimento dell’incarico professionale'],
      note: isPF ? nonApplicabilePF : linesOf('visura'),
    },
    {
      checked: !isPF && !haVisura && haAttiCostitutivi,
      doc: ['Atti costitutivi e delibere per i soggetti/enti che non sono tenuti all’iscrizione al Registro delle Imprese'],
      oss: ['Per verificare esistenza e ampiezza dei poteri di rappresentanza del soggetto che agisce per conto dell’ente nel conferimento dell’incarico professionale'],
      note: isPF
        ? nonApplicabilePF
        : haVisura
          ? ['Non applicabile — soggetto iscritto al Registro delle Imprese: poteri di rappresentanza verificati tramite visura (riga precedente).']
          : linesOf('atti_costitutivi'),
    },
    {
      checked: !!checks['cl_codice_fiscale'],
      doc: ['Attestazione codice fiscale e (eventuale) partita IVA'],
      oss: [],
      note: noteCF,
    },
    {
      checked: !!checks['cl_mandato'],
      doc: ['Mandato (lettera di incarico) professionale'],
      oss: ['Al fine di documentare la data di inizio dell’incarico professionale, copia del mandato professionale scritto e relativa accettazione da parte del Cliente'],
      note: noteMandato,
    },
    {
      checked: !!checks['cl_scheda_av3'],
      doc: ['Scheda di adeguata verifica (ai fini della dimostrazione dell’avvenuto adempimento dei relativi obblighi)'],
      oss: ['Si consiglia l’adozione del modello AV.3 delle presenti Linee Guida al fine di documentare:'],
      ossBullets: [
        'l’identificazione del Cliente',
        'l’identificazione del Titolare effettivo',
        'l’acquisizione e valutazione di informazioni su scopo e natura del rapporto continuativo o della prestazione professionale',
        'il controllo costante',
      ],
      note: noteAv3,
    },
    {
      checked: !!checks['cl_dichiarazione_av4'],
      doc: ['Dichiarazione antiriciclaggio resa dal Cliente ex art. 22 D.Lgs. 231/2007'],
      oss: ['Si consiglia l’adozione del modello AV.4 delle presenti Linee Guida'],
      note: linesOf('dichiarazione_av4'),
    },
    {
      checked: !!checks['cl_scheda_av1'],
      doc: ['Scheda di determinazione del rischio effettivo ex art. 17 D.Lgs. 231/2007'],
      oss: ['Si consiglia l’adozione del modello AV.1 delle presenti Linee Guida'],
      note: noteAv1,
    },
    {
      checked: !!checks['cl_attestazione_av5'],
      doc: ['Attestazione per l’esecuzione dell’obbligo di adeguata verifica da parte di terzi ex art. 26, D.Lgs. 231/2007'],
      oss: ['Si consiglia l’adozione del modello AV.5 delle presenti Linee Guida'],
      note: linesOf('attestazione_av5'),
    },
    {
      checked: !!checks['cl_doc_semplificati_rafforzati'],
      doc: ['Documentazione in base alla quale si è verificata la possibilità (o la necessità) di applicare obblighi semplificati (o rafforzati) di adeguata verifica della clientela'],
      oss: ['Vedasi articoli 23, 24 e 25, D.Lgs. 231/2007 e Regola Tecnica n.2 '],
      note: noteSempRaff,
    },
    {
      checked: !!checks['cl_dichiarazione_penale'],
      doc: ['Dichiarazione sostitutiva di certificazioni e di atti notori o certificato del Tribunale in merito ad eventuali condanne e procedimenti penali in corso'],
      oss: ['Nel caso si venga a conoscenza di condanne o procedimenti in corso a carico del Cliente/titolare effettivo e si ritenga necessario documentare l’esclusione del collegamento tra le imputazioni e la prestazione professionale richiesta'],
      note: linesOf('dichiarazione_penale'),
    },
    {
      checked: !!checks['cl_esiti_ricerche'],
      doc: ['Esiti di ricerche su internet o in apposite banche dati del nominativo del Cliente, del soggetto che agisce per conto del Cliente e degli eventuali titolari effettivi'],
      oss: ['Nel caso si ritenga utile verificare la presenza di eventuali condanne o notizie pregiudizievoli sul Cliente/titolare effettivo per valutarne l’eventuale connessione con la prestazione professionale richiesta'],
      note: linesOf('esiti_ricerche'),
    },
    {
      checked: !!checks['cl_consistenza_patrimoniale'],
      doc: ['Documentazione o attestazioni comprovanti la consistenza patrimoniale e/o la capacità di credito del Cliente (p.e. ultima dichiarazione dei redditi, ultimo bilancio approvato, lettera di referenze di un Istituto di Credito, lettera di presentazione di un soggetto sottoposto alla normativa antiriciclaggio, ecc.).'],
      oss: ['Ove la prestazione professionale comporti una consistente movimentazione di mezzi di pagamento e si ritenga necessario approfondire e documentare la coerenza delle disponibilità con il profilo economico/patrimoniale del Cliente'],
      note: linesOf('consistenza_patrimoniale', 'bilancio'),
    },
    {
      checked: !!checks['cl_visura_nominativa'],
      doc: ['Visura camerale nominativa completa per codice fiscale per la verifica delle cariche sociali (amministratore e socio), del bollettino dei protesti e dell’assoggettamento a procedure concorsuali del legale rappresentante e degli eventuali titolari effettivi'],
      oss: ['Ove possa essere utile verificare o approfondire la posizione soggettiva della persona (Cliente, esecutore, titolare effettivo)'],
      note: linesOf('visura_nominativa'),
    },
    {
      checked: !!checks['cl_posizione_giuridica'],
      doc: ['Documentazione che comprovi l’esame della posizione giuridica del Cliente o l’espletamento di compiti di difesa o rappresentanza davanti a un’Autorità giudiziaria compresa la consulenza sull’eventualità di intentare o evitare il procedimento (e copia dell’incarico professionale conferito)'],
      oss: ['Ai fini del termine per la verifica dell’identità del Cliente (comma 4, art.18, D.Lgs. 231/2007) e dell’esonero da SOS (comma 5, art.35, D.Lgs. 231/2007)'],
      note: linesOf('posizione_giuridica'),
    },
    {
      checked: !!checks['cl_sanzioni_check'],
      doc: ['Rapporti/documentazione circa un eventuale nominativo rilevante ai fini antiterrorismo'],
      oss: ['Nel caso si renda necessaria una verifica del nominativo del Cliente rispetto alle liste delle persone e degli enti associati ad attività di finanziamento del terrorismo o destinatari di misure di congelamento'],
      note: noteSanzioni,
    },
    {
      checked: notePubbliciRegistri.length > 0,
      doc: ['Documenti, estratti da pubblici registri o annotazioni che il Professionista ritenga opportuno conservare ai fini della normativa antiriciclaggio e di finanziamento del terrorismo, in particolare ai fini della valutazione dei rischi e delle segnalazioni di operazioni sospette'],
      oss: ['Può essere utile inserire dati, documenti e annotazioni non espressamente richiesti dalle norme, ma che costituiscono un supporto alla valutazione del rischio e alle motivazioni che hanno condotto, o meno, alla segnalazione di un’operazione sospetta'],
      note: notePubbliciRegistri,
    },
    {
      checked: !!checks['cl_controllo_costante'],
      doc: ['Documenti relativi alle modifiche anagrafiche (ove intervenute in vigenza di incarico professionale) o altri documenti per il controllo costante'],
      oss: ['Del Cliente persona fisica, del Cliente società, del soggetto che ha rappresentato la società nell’incarico professionale, del titolare effettivo e dell’eventuale esecutore; se le modifiche sono tali da comportare una variazione del livello di rischio, risulta opportuno aggiornare la scheda di valutazione del rischio antiriciclaggio/antiterrorismo'],
      note: noteControllo,
    },
    {
      checked: !!(checks['cl_mezzi_pagamento'] || checks['cl_provenienza_fondi']),
      doc: ['Documenti riferiti alle “operazioni” secondo la definizione dell’art. 1 lett. t) D.Lgs. 231/2007'],
      oss: [],
      note: noteOperazioni,
    },
    {
      checked: altriDocumenti.length > 0,
      // Il modello lascia 4 righe punteggiate sotto "(specificare):": le riempiamo con le
      // voci disponibili e teniamo le restanti libere per la compilazione manuale.
      doc: [
        'Altra documentazione ritenuta opportuna a seguito di valutazioni/considerazioni del Professionista (specificare):',
        ...altriDocumenti.map(v => `- ${v}`),
        ...Array.from({ length: Math.max(0, 4 - altriDocumenti.length) }, () => '.....................................................................'),
      ],
      oss: ['Ogni ulteriore documento o traccia di informazione necessaria o utile per l’adeguata verifica, ovvero per altri presidi antiriciclaggio'],
      note: [],
    },
    {
      checked: docsOf('astensione_av6').length > 0,
      doc: ['Dichiarazione di astensione del Professionista (art. 42 D.Lgs. 231/2007) '],
      oss: ['Si consiglia l’adozione del modello AV.6 delle presenti Linee Guida'],
      note: linesOf('astensione_av6'),
    },
    {
      checked: !!incarico.data_fine,
      doc: ['Documentazione relativa alla cessazione della prestazione professionale'],
      oss: ['Eventuale lettera/comunicazione di revoca o di rinuncia all’incarico. Copia della cancellazione partita IVA/codice fiscale, cessazione dal Registro delle Imprese, decreto di estinzione, ecc. in capo al Cliente'],
      note: noteCessazione,
    },
  ];
  ...
  const headerRow = new TableRow({
    tableHeader: true,
    children: [
      av2Cell([av2Para([av2Text('(X)')])], AV2LG_COLS[0], { vAlign: VerticalAlign.CENTER }),
      av2Cell([av2Para([av2Text('DOCUMENTAZIONE', { bold: true })], { align: AlignmentType.CENTER })], AV2LG_COLS[1], { vAlign: VerticalAlign.CENTER, shading: AV2LG_SHD_DOC }),
      av2Cell([av2Para([av2Text('OSSERVAZIONI', { bold: true })], { align: AlignmentType.CENTER })], AV2LG_COLS[2], { vAlign: VerticalAlign.CENTER, shading: AV2LG_SHD_OSS }),
      av2Cell([av2Para([av2Text('ANNOTAZIONI PROFESSIONISTA', { bold: true })], { align: AlignmentType.CENTER })], AV2LG_COLS[3], { vAlign: VerticalAlign.CENTER }),
    ],
  });

  const bodyRows = righe.map(r => {
    // Colonna (X): casella spuntata/vuota, oppure cella vuota per le righe separatore
    const cellaCheck = r.checked === null
      ? av2Cell([], AV2LG_COLS[0])
      : new TableCell({
          width: { size: AV2LG_COLS[0], type: WidthType.DXA },
          verticalAlign: VerticalAlign.CENTER,
          margins: { left: 40, right: 40, top: 40, bottom: 40 },
          children: [av2Para([av2Text(getCheckbox(r.checked), { size: 20, bold: r.checked })], { align: AlignmentType.CENTER })],
        });

    const paragrafiDoc = r.doc.map((t, i) => av2Para([av2Text(t, { bold: r.docBold && i === 0 })]));
    const paragrafiOss = [
      ...r.oss.map((t, i) => av2Para([av2Text(t, { bold: r.ossLastBold && i === r.oss.length - 1 })])),
      ...(r.ossBullets || []).map(t =>
        new Paragraph({
          children: [av2Text(`•  ${t}`)],
          alignment: AlignmentType.JUSTIFIED,
    ...
    return new TableRow({
      children: [
        cellaCheck,
        av2Cell(paragrafiDoc, AV2LG_COLS[1], { shading: r.plain ? undefined : AV2LG_SHD_DOC }),
        av2Cell(paragrafiOss, AV2LG_COLS[2], { shading: r.plain ? undefined : AV2LG_SHD_OSS }),
        av2Cell(r.note.map(t => av2Para([av2Text(t)])), AV2LG_COLS[3]),
      ],
    });
  });
  ...
  elems.push(
    av2Para([av2Text('AV.2 – CHECK-LIST AI FINI DELLA FORMAZIONE DEL FASCICOLO DEL CLIENTE', { bold: true, size: 26 })], { before: 200, after: 120 })
  );

  // Identificazione precompilata: il riepilogo è prodotto per singolo incarico
  const clienteLabel = cliente.ragione_sociale || cliente.rappresentante_legale || '';
  const idIncarico = [incarico.codice_incarico, incarico.descrizione || incarico.scopo_natura].filter(Boolean).join(' — ');
  if (clienteLabel) elems.push(av2Para([av2Text('Cliente: ', { size: 18 }), av2Text(clienteLabel, { size: 18, bold: true })], { after: 40 }));
  if (idIncarico) elems.push(av2Para([av2Text('Incarico: ', { size: 18 }), av2Text(idIncarico, { size: 18, bold: true })], { after: 40 }));
  elems.push(
    classeFinale
      ? av2Para([av2Text('Adeguata verifica: ', { size: 18 }), av2Text(`${getTipoAdeguataVerifica(classeFinale)} (classe ${classeFinale})`, { size: 18, bold: true })], { after: 200 })
      : av2Para([av2Text('Adeguata verifica: nessuna valutazione del rischio presente a sistema.', { size: 18, italic: true })], { after: 200 })
  );

  elems.push(
    new Table({
      width: { size: AV2LG_TABLE_W, type: WidthType.DXA },
      columnWidths: [...AV2LG_COLS],
      borders: TABLE_BORDERS,
  ...
  const doc = new Document({
    sections: [
      {
        properties: {
          // Margini del modello ALLEGATI_Linee_Guida.docx
          page: { margin: { top: 1560, right: 1134, bottom: 1134, left: 1134 } },
        },
        children: elems,
      },
    ],
  ...
  const blob = await Packer.toBlob(doc);
  const filename = `AV2_Checklist_Completa_${incarico.codice_incarico || today.replace(/\//g, '-')}.docx`;
  return { blob, filename };
}

export async function generateAndDownloadDOCX_AV2Completo(data: AMLData, options: AV2Options): Promise<void> {
  const { blob, filename } = await generateBlobDOCX_AV2Completo(data, options);
  saveAs(blob, filename);
}
