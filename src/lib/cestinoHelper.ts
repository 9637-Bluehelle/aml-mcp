...
export interface CestinoEntry {
  ...
  /** Tentativi falliti dello svuotamento automatico (0 = mai fallito). */
  purge_tentativi?: number;
  /** Ultimo errore dello svuotamento automatico, se c'è stato. */
  purge_errore?: string | null;
  /** Quando si è verificato l'ultimo errore di svuotamento automatico. */
  purge_errore_at?: string | null;
}

/**
 * Esito della pulizia dei file dopo un'eliminazione definitiva.
 *
 * I dati nel database sono già stati cancellati quando questa fase parte: un
 * file che resta indietro non è più raggiungibile da nulla, quindi va detto e
 * non ignorato. L'eliminazione in sé resta comunque valida.
 */
export interface EsitoFile {
  /** File che lo Storage non ha rimosso. */
  nonRimossi: number;
  /** Messaggio dell'errore; null se è andato tutto bene. */
  errore: string | null;
}
...
/**
 * Apre un avviso di piattaforma per i file rimasti nello Storage.
 * Visibile al solo superadmin (`automatica`), come per gli altri avvisi tecnici:
 * l'utente non può farci nulla, ma qualcuno deve sapere che ci sono file orfani.
 * Non deve mai far fallire l'eliminazione, che a questo punto è già avvenuta.
 */
async function segnalaFileOrfani(paths: string[], errore: string): Promise<void> {
  try {
    ...
    await supabase.from('segnalazioni').insert({
      user_id: user.id,
      categoria: 'bug',
      oggetto: '[AUTO] Cestino: file non rimossi dallo Storage',
      descrizione:
        `L'eliminazione definitiva è riuscita nel database, ma la rimozione di ${paths.length} `
        + `file dallo Storage è fallita: ${errore}\n\n`
        + `I file restano occupati senza essere più referenziati da alcun record.\n`
        + `Percorsi interessati:\n${paths.slice(0, 50).join('\n')}`
        + `${paths.length > 50 ? `\n...e altri ${paths.length - 50}` : ''}\n`
        + `Timestamp: ${new Date().toISOString()}`,
      sezione: 'Cestino - Eliminazione definitiva',
      automatica: true,
    });
  } catch {
    /* l'avviso è un di più: se non parte, l'eliminazione resta comunque valida */
  }
}

/**
 * Rimuove i file dallo Storage e riporta cosa è andato storto.
 *
 * `storage.remove()` NON solleva eccezioni sugli errori applicativi: li mette in
 * `error` nell'oggetto restituito. Il vecchio try/catch attorno alla chiamata
 * non li intercettava nemmeno, quindi un fallimento spariva del tutto.
 */
async function rimuoviFiles(paths: string[] | null | undefined): Promise<EsitoFile> {
  const validi = (paths ?? []).filter(Boolean);
  if (validi.length === 0) return { nonRimossi: 0, errore: null };
  try {
    const { error } = await supabase.storage.from('file_allegati').remove(validi);
    if (error) throw error;
    return { nonRimossi: 0, errore: null };
  } catch (e) {
    const errore = (e as Error)?.message || String(e);
    await segnalaFileOrfani(validi, errore);
    return { nonRimossi: validi.length, errore };
  }
}

/** Cancellazione definitiva di una singola voce del cestino (+ pulizia Storage). */
export async function svuotaElemento(cestinoId: string): Promise<EsitoFile> {
  const { data, error } = await supabase.rpc('svuota_elemento', { p_cestino_id: cestinoId });
  if (error) throw error;
  const esito = await rimuoviFiles(data?.file_paths);
  await addUserLog('Eliminato definitivamente dal cestino');
  notifyCestinoChanged();
  return esito;
}

/** Svuota l'intero cestino dello studio (+ pulizia Storage). */
export async function svuotaCestino(): Promise<EsitoFile & { eliminati: number }> {
  const { data, error } = await supabase.rpc('svuota_cestino');
  if (error) throw error;
  const esito = await rimuoviFiles(data?.file_paths);
  await addUserLog('Svuotato il cestino');
  notifyCestinoChanged();
  return { ...esito, eliminati: data?.eliminati ?? 0 };
}
 ...
 *
 * NB: la cancellazione NON avviene all'istante esatto della scadenza. Un cron
 * gira ogni notte alle 03:00 ed elimina solo i batch la cui scadenza teorica
 * (`deleted_at + N giorni`) è GIÀ passata. La `data` restituita è quindi la
 * prima pulizia notturna a partire dalla scadenza — non la scadenza stessa —
 * così l'etichetta non promette un orario che il purge non rispetterebbe.
 */
export function calcolaPurge(
  ...
  // Scadenza teorica; il purge effettivo è la prima pulizia notturna (03:00) a
  // partire da questo istante.
  const scadenza = new Date(base + giorni * giorno);
  const data = new Date(scadenza);
  data.setHours(3, 0, 0, 0);
  if (data.getTime() < scadenza.getTime()) data.setDate(data.getDate() + 1);
...
export async function caricaContestoVoci(
  ...
  // --- Tipi legati direttamente a un incarico (RT2/RT4/SOS) ---
...
> = {
  ...
  controlli_costanti: {
    etichetta: 'Controlli costanti (RT4)', ordine: 7,
 ...
 * incarichi → RT2 → RT4 → SOS → documenti); eventuali chiavi non previste
