import { retryQuery } from './supabaseRetry';
...
export async function fetchAllInBatches<T>(
  ...
  for (;;) {
    ...
    // retryQuery assorbe i 503/PGRST002 transitori (DB in risveglio o schema
    // cache in ricarica dopo migrazioni) ricostruendo la query a ogni tentativo.
    const { data, error } = await retryQuery(() => buildQuery().range(from, to));
