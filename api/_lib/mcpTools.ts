...
import { z } from 'zod';
import type { WizardData } from '../../src/components/cliente-wizard/types.js';
...
export const creaBozzaClienteSchema = {
  ...
  data_nascita_pf: z.string().optional().describe('[persona_fisica] Data di nascita (dd/mm/yyyy o ISO yyyy-mm-dd).'),
  ...
  data_nascita_prof: z.string().optional().describe('[professionista] Data di nascita (dd/mm/yyyy o ISO).'),
...
/** Mappa gli argomenti del tool su WizardData (pass-through: i nomi combaciano) + required. */
export function mapArgsToWizardData(args: Record<string, unknown>): WizardData {
  return { titolari_effettivi: [], ...args } as unknown as WizardData;
}
...
export const creaSoggettoSchema = {
  ...
  data_nascita: z.string().optional().describe('Data di nascita (dd/mm/yyyy o ISO), solo persona fisica.'),
...
export const creaIncaricoSchema = {
  cliente_id: z.string().uuid().describe('UUID del cliente a cui associare l\'incarico (da lista_clienti/leggi_cliente).'),
  ...
  data_inizio: z.string().describe('Data di inizio (dd/mm/yyyy o ISO yyyy-mm-dd).'),
  data_fine: z.string().optional().describe('Data di fine (dd/mm/yyyy o ISO), se nota.'),
...
export const creaValutazioneSchema = {
  incarico_id: z.string().uuid().describe('UUID dell\'incarico da valutare (da lista_incarichi).'),
  tabella_a: z.object({
